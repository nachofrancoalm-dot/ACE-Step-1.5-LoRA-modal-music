...
% Redefinir \makelicense para suprimir el bloque BibTeX manteniendo la licencia CC
\makeatletter
\renewcommand\makelicense{{%
    \footnotesize
    \clearpage
    \thispagestyle{empty}
    ~\vfill
...
%
\title{LoRA para control de modos musicales en ACE-Step 1.5}
...
donde $W \in \mathbb{R}^{d \times k}$ son los pesos originales congelados (con $d$ la dimensi\'on de salida y $k$ la dimensi\'on de entrada de la capa lineal proyectada), $B \in \mathbb{R}^{d \times r}$ y $A \in \mathbb{R}^{r \times k}$ son las matrices LoRA entrenables, $r$ es el rango y $\alpha$ el factor de escala. Solo $A$ y $B$ reciben gradiente; el resto del modelo no.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{5.2cm}p{3.3cm}p{4.5cm}}
        ...
        \texttt{rank\_checkpoints\_per\_}\newline\texttt{experiment.py}
            & Ranking determinista de checkpoints
            & CSV de m\'etricas $\to$ lista ordenada de candidatos \\
        \texttt{prepare\_manual\_eval\_}\newline\texttt{shortlist.py}
...
La fase autom\'atica se apoy\'o en el \'indice compuesto \texttt{auto\_score\_0\_1}, calculado a partir de cinco m\'etricas extra\'idas del \emph{cromagrama} del audio generado. El cromagrama es una representaci\'on derivada del espectro: divide cada instante del audio en las 12 clases de altura del sistema tonal occidental (C, C\#, D, \dots, B) y mide cu\'anta energ\'ia sonora hay en cada una. El audio se procesa por ventanas temporales cortas, de modo que se obtiene un \emph{vector de chroma} (12 valores, uno por nota) para cada ventana. La Figura~\ref{fig:chroma-vector} muestra un ejemplo de vector de chroma simulado para Re d\'orico, donde las notas diat\'onicas concentran la mayor parte de la energ\'ia y la t\'onica destaca sobre el resto. Sobre esta representaci\'on se calculan las siguientes m\'etricas:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/chroma_vector_dorico.png}
    \caption{Ejemplo de vector de chroma para Re d\'orico (valores simulados). Cada barra representa la energ\'ia relativa de una clase de altura; la t\'onica (D, rojo) domina, las dem\'as notas diat\'onicas (azul) concentran la mayor parte de la energ\'ia restante, y las notas no diat\'onicas (gris) presentan valores muy bajos. La l\'inea de puntos indica la plantilla binaria diat\'onica utilizada en la m\'etrica $s_m$. La 6\textordfeminine\ mayor (Si) es la nota caracter\'istica que distingue al d\'orico del e\'olico.}
    \label{fig:chroma-vector}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Similitud con plantilla modal ($s_m$).} Similitud coseno entre el vector de chroma medio del audio y la plantilla binaria del modo objetivo (1 en las posiciones diat\'onicas, 0 en el resto), inspirada en los perfiles tonales de Krumhansl--Kessler~\cite{krumhansl1982}, que son distribuciones de prominencia perceptiva de las 12 clases de altura para cada tonalidad mayor y menor, obtenidas experimentalmente mediante valoraciones de oyentes.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{4.2cm}p{5.5cm}}
        ...
        Centro tonal
            ...
            & 1: ningún CT; 2: atisbo de t\'onica, muy inestable; 3: CT intermitente pero identificable; 4: CT claro con alguna desviaci\'on; 5: CT claro y sostenido \\
        \addlinespace
        Color modal
            & La sonoridad refleja el modo objetivo (p. ej. la 6\textordfeminine\ mayor en d\'orico)
            & 1: ningún rasgo modal; 2: algún rasgo aislado, difuso; 3: algún rasgo pero ambiguo; 4: color modal reconocible aunque no sistem\'atico; 5: color modal claramente identificable \\
        \addlinespace
        No modulaci\'on
            & La pieza se mantiene en el modo objetivo sin cambiar de tonalidad
            & 1: modulaciones frecuentes o graves; 2: varias desviaciones o una grave; 3: alguna desviaci\'on puntual; 4: muy estable con deslizamientos breves; 5: sin modulaciones apreciables \\
        \addlinespace
        Coherencia musical
            & La m\'usica es musicalmente coherente y escuchable (tim\-bre, ritmo, estructura)
            & 1: muy degradada o incoherente; 2: degradaci\'on notable, escuchabilidad limitada; 3: aceptable con partes irregulares; 4: fluida, algo inferior al modelo base; 5: fluida y comparable al modelo base \\
        \bottomrule
    \end{tabular}
\end{table}
...
\section{An\'alisis de tasas de \'exito por umbral}
\label{s:tasas-exito-umbral}

Las tablas presentadas hasta ahora resumen la evaluaci\'on manual mediante \textbf{promedios} por configuraci\'on y por modo. Esta forma de agregaci\'on es \'util para comparar tendencias globales, pero presenta una limitaci\'on conocida: un mismo promedio puede corresponder a distribuciones muy distintas. Una puntuaci\'on media de 2.5 sobre 5 puede surgir de un conjunto de audios homog\'eneamente mediocres o de una mezcla de audios excelentes con otros muy d\'ebiles. Para distinguir entre ambos casos se introduce un segundo punto de vista, complementario al promedio, basado en \textbf{tasas de \'exito por umbral}.

Para cada audio y criterio de la r\'ubrica se define un umbral binario:

\begin{itemize}
    \item \textbf{Umbral permisivo} ($\geq 2$ en escala 1--5, equivalente a $\geq 10$ en el total 0--20). Marca aquellos audios en los que el modelo muestra \emph{algo} del rasgo evaluado, m\'as all\'a del rango "ning\'un rasgo" descrito en la r\'ubrica.
    \item \textbf{Umbral estricto} ($\geq 3$ en escala 1--5, equivalente a $\geq 12$ en el total 0--20). Marca audios en los que el rasgo evaluado est\'a presente de forma reconocible, aunque a\'un sea ambigua.
\end{itemize}

A partir de estos umbrales se calcula, por cada configuraci\'on \texttt{Run + Checkpoint}, el porcentaje de audios que superan cada nivel. La elecci\'on de dos umbrales en paralelo evita comprometerse con un corte arbitrario \'unico y permite distinguir configuraciones donde el modelo \emph{acierta de forma consistente} de aquellas donde simplemente \emph{no falla del todo}.

\subsection{Ranking por criterio}

La Figura~\ref{fig:exito-color-modal} muestra el top~10 de configuraciones por tasa de \'exito en \textbf{color modal}, el criterio m\'as directamente alineado con el objetivo del proyecto. Las dos barras agrupadas por configuraci\'on permiten leer simult\'aneamente cu\'antos audios superan el umbral permisivo (alguna traza modal) y cu\'antos superan el estricto (rasgo modal reconocible).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_top10_color_modal.png}
    \caption{Top~10 de configuraciones por porcentaje de audios que superan el umbral en \textbf{color modal}. Se muestran lado a lado el umbral permisivo ($\geq 2$) y el estricto ($\geq 3$).}
    \label{fig:exito-color-modal}
\end{figure}

La lectura es directa. \texttt{r128\_a256\_d005} en su checkpoint temprano lidera el ranking estricto con un 57.1\,\% de audios reconocibles, seguido por la variante \texttt{Prompt solo modo} con 42.9\,\% y un grupo de tres configuraciones empatadas a 35.7\,\% que incluye los dos checkpoints del experimento de confirmaci\'on (epoch~140 y epoch~160). El experimento de confirmaci\'on no domina el ranking global, sino que se sit\'ua en la zona media del top~10. Este resultado es coherente con el dise\~no del experimento: la configuraci\'on se eligi\'o para atacar el sesgo en modos menores espec\'ificos, no para maximizar el promedio sobre los siete modos.

Las figuras correspondientes para el resto de criterios (centro tonal, no modulaci\'on y coherencia) y para el total agregado se presentan en el Ap\'endice~\ref{app:rankings-completos} (figuras~\ref{fig:exito-centro-tonal} a \ref{fig:exito-total}). El patr\'on global se sintetiza en el mapa de calor de la Figura~\ref{fig:exito-heatmap}, que permite leer de un vistazo qu\'e configuraciones destacan en cada criterio.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_heatmap.png}
    \caption{Mapa de calor del porcentaje de \'exito estricto ($\geq 3$ en criterios 1--5; $\geq 12$ en total 0--20) para las diez configuraciones con mejor promedio. Cada columna corresponde a un criterio de la r\'ubrica m\'as el total agregado.}
    \label{fig:exito-heatmap}
\end{figure}

\subsection{Lectura por modo: el caso del d\'orico}

El ranking global por configuraci\'on enmascara una observaci\'on cr\'itica para la interpretaci\'on del experimento de confirmaci\'on. Cuando el c\'alculo se realiza por \texttt{Run + Checkpoint + Modo}, el experimento de confirmaci\'on \texttt{confirm\_r32\_a32\_lr1e-4} alcanza el \textbf{100\,\% de \'exito estricto en color modal sobre los audios d\'oricos}, en sus dos checkpoints evaluados (epoch~140 y epoch~160). Esta es la \'unica configuraci\'on del estudio que produce dos de dos audios d\'oricos con color modal reconocible, frente al 0\,\% que obtienen tanto \texttt{run\_D} como la mayor\'ia de configuraciones de la b\'usqueda en ese mismo modo.

Esta lectura no contradice el promedio mostrado en la Tabla~\ref{tab:confirm-comparativa} ---donde el confirm queda por debajo de run\_D en el global--- sino que la complementa: lo que el promedio describe como \textquotedblleft ligero retroceso global con mejora en d\'orico\textquotedblright\ se traduce, en t\'erminos de tasa de \'exito, en \textquotedblleft el confirm es la \'unica configuraci\'on capaz de superar el umbral estricto en todos sus audios d\'oricos\textquotedblright. La diferencia entre ambos puntos de vista ilustra el valor de presentar el resultado en dos formas: el promedio cuantifica la distancia, la tasa binaria cuantifica la consistencia.

En el extremo opuesto, el modo locrio ofrece la lectura m\'as severa: ninguna configuraci\'on del estudio supera el 0\,\% de \'exito estricto sobre los audios locrios, lo que refuerza num\'ericamente la observaci\'on cualitativa de que este modo permanece sin control efectivo en todas las configuraciones evaluadas.

\subsection{Interpretaci\'on}

La introducci\'on de tasas de \'exito por umbral aporta tres elementos a la discusi\'on:

\begin{enumerate}
    \item \textbf{Separa consistencia de magnitud.} El promedio premia configuraciones con muchos audios moderadamente buenos; la tasa estricta premia configuraciones que producen aciertos claros aunque sean menos. Ambos puntos de vista son leg\'itimos y describen propiedades distintas de un mismo conjunto de resultados.
    \item \textbf{Refuerza la metodolog\'ia de evaluaci\'on dual.} El cribado autom\'atico determinista filtra el espacio de configuraciones; la r\'ubrica manual con promedio describe tendencias; la lectura por umbral cuantifica la fiabilidad. Las tres capas son complementarias y reflejan decisiones independientes.
    \item \textbf{Soporta la conclusi\'on del experimento de confirmaci\'on con evidencia binaria.} El hallazgo de que el confirm produce el 100\,\% de audios d\'oricos con color modal reconocible es un resultado m\'as fuerte que la simple mejora de promedio, ya que no depende de una diferencia num\'erica peque\~na sino de una propiedad cualitativa sin contraejemplo.
\end{enumerate}

Las observaciones del an\'alisis cualitativo del p\'arrafo anterior y de la secci\'on~\ref{s:analisis-cualitativo-modos} encuentran aqu\'i un soporte cuantitativo binario que las hace m\'as compactas de comunicar al lector.
...
A modo de ilustraci\'on, los tres prompts para el modo d\'orico son:

\begin{quote}\small
\textbf{B (detallado):} \texttt{dorian mode, jazz-fusion instrumental, D dorian, electric piano lead, fretless bass, brushed drums, 96 BPM, minor with hopeful color, natural 6th flavor, stable tonic on D, no modulation, no vocals}

\medskip
\textbf{Modo+instrumento:} \texttt{dorian mode, instrumental, electric piano, fretless bass, brushed drums, no vocals}

\medskip
\textbf{Modo solo:} \texttt{dorian mode}
\end{quote}
...
En conjunto, los objetivos del proyecto se consideran alcanzados: se ha establecido una metodolog\'ia experimental completa y reproducible, se han identificado las configuraciones LoRA m\'as prometedoras mediante b\'usqueda sistem\'atica, y se ha validado emp\'iricamente que la configuraci\'on \'optima reduce de forma medible el sesgo en modos menores, con avances especialmente significativos en el modo d\'orico (+12\,pp en evaluaci\'on perceptiva). El an\'alisis complementario por tasa de \'exito por umbral (secci\'on~\ref{s:tasas-exito-umbral}) refuerza esta conclusi\'on con evidencia binaria: el experimento de confirmaci\'on es la \'unica configuraci\'on del estudio que produce el 100\,\% de audios d\'oricos con color modal reconocible bajo el umbral estricto, mientras que en el ranking global se mantiene en posiciones intermedias, coherente con su dise\~no focalizado.
...
\chapter{Rankings completos por tasa de \'exito}
\label{app:rankings-completos}

Este ap\'endice recoge las figuras complementarias a la secci\'on~\ref{s:tasas-exito-umbral}, mostrando el top~10 de configuraciones por tasa de \'exito en los criterios de la r\'ubrica no incluidos en el cuerpo principal del cap\'itulo, as\'i como en la puntuaci\'on total agregada.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_top10_centro_tonal.png}
    \caption{Top~10 de configuraciones por porcentaje de \'exito en \textbf{centro tonal}.}
    \label{fig:exito-centro-tonal}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_top10_no_modulacion.png}
    \caption{Top~10 de configuraciones por porcentaje de \'exito en \textbf{no modulaci\'on}.}
    \label{fig:exito-no-modulacion}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_top10_coherencia.png}
    \caption{Top~10 de configuraciones por porcentaje de \'exito en \textbf{coherencia musical}.}
    \label{fig:exito-coherencia}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/exito_top10_total.png}
    \caption{Top~10 de configuraciones por porcentaje de audios que superan el umbral en \textbf{total agregado} (umbrales permisivo $\geq 10/20$ y estricto $\geq 12/20$).}
    \label{fig:exito-total}
\end{figure}
