...
%
\title{Fine-Tuning Modal sobre modelo Ace-Step de generación Musical}
\author{Ignacio Franco Almendárez}
\bibauthor{Francisco Serradilla}
...
\abstract{spanish}{
    ...
    Este trabajo aborda la adaptación y el estudio del modelo generativo musical \textit{ACE-Step}, 
    
    Este trabajo aborda la adaptación del modelo generativo musical \textit{ACE-Step}, condicionado por texto, con el objetivo de dotarlo de un control explícito sobre la modalidad (jónico, dórico, frigio, lidio, mixolidio, eólico y locrio) sin necesidad de reentrenar el modelo completo. Para ello se construye un conjunto de datos curado de 102 fragmentos musicales etiquetados por modo, con duraciones de 30 a 60 segundos y un reparto no perfectamente balanceado entre clases.
    
    La metodología se basa en \textit{fine-tuning} mediante \textit{LoRA} (\textit{Low-Rank Adaptation}), entrenando únicamente adaptadores de bajo rango sobre módulos seleccionados del transformador del modelo, de forma que el resultado sea un conjunto de ficheros de pesos ligeros y fácilmente desplegables. Adicionalmente, se ha adaptado el repositorio para facilitar el entrenamiento y la exportación de adaptadores LoRA sin generar \textit{checkpoints} completos de tamaño excesivo, y para mejorar la robustez de la inferencia y el guardado de audio en distintos entornos.
    
    La evaluación se realiza mediante comparativas A/B controladas (misma semilla, instrumentación y parámetros, variando únicamente el \textit{tag} de modo) y con el apoyo de una métrica heurística basada en cromas para estimar consistencia modal, complementada con escucha crítica. Los resultados iniciales muestran que imponer una propiedad armónica global como el modo requiere un equilibrio cuidadoso entre la capacidad del adaptador, el peso de inyección en inferencia y el diseño del \textit{prompt}, evitando degradaciones tímbricas o comportamientos inestables.
}
\keywords{spanish}{
    Cuatro o cinco;
    Expresiones clave;
...
\newacronym[
    description={S.P.E.C.I.A.L es la sigla usada para los atributos de
        Fuerza (\textbf{S}trength), Percepción (\textbf{P}erception),
        Resistencia (\textbf{E}ndurance), Carisma (\textbf{C}harisma),
        Inteligencia (\textbf{I}ntelligence), Agilidad (\textbf{A}gility)
        y Suerte (\textbf{L}uck)
    },
    ]{special}{SPECIAL}{\textit{Strength, Perception, Endurance,
...
Y el principio es el fichero \texttt{main.tex}, por lo que vamos a ver
las primeras líneasPor el principio, y con cuidado. Esto es, con el fichero \texttt{main.tex}. Veamos la primera línea del fichero (listado~\ref{lst:starting-report}).

\lstinputlisting[
    language=tex,
    firstline=1,
    lastline=5,
    caption=Primeras líneas del fichero \texttt{main.tex},label=lst:starting-report
]{main.tex}
...
el mismo fichero (\texttt{main.tex}), pero es muy común escribirlo en
